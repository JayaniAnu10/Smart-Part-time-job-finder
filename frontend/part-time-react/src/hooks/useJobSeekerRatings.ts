...
export interface RatingDetailData {
  ...
  jobTitle: string;
  raterId: string;
  raterName: string; // This is the employer/company name
...
const useJobSeekerRatings = (id: string, page: number = 0, size: number = 10) => {
  ...
  return useQuery<RatingsPageResponse, Error>({
    queryKey: ["jobseeker", "ratings", id, page, size],
    queryFn: apiClient.get,
    enabled: !!id,
  });
};

export default useJobSeekerRatings;
